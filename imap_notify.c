...
static SylPluginInfo info = {
	"IMAP NOTIFY Plug-in for windows",
	"1.1.1",
	"CLehner+ f16kv26",
	"IMAP NOTIFY implementation for Sylpheed for windows"
};
...
static void show_notifications(FolderItem *item) {
	...
	gboolean notifications_wanted =
		prefs_common_get()->enable_newmsg_notify_window ||
		(prefs_common_get()->enable_newmsg_notify_sound &&
		 prefs_common_get()->newmsg_notify_sound) ||
		(prefs_common_get()->enable_newmsg_notify &&
		 prefs_common_get()->newmsg_notify_cmd);
	...
	if (session->use_idle) {
		...
		if (prefs_common_get()->enable_newmsg_notify_window)
...
static void display_summaries(void)
{
	...
	syl_plugin_notification_window_open(title, str->str,
			prefs_common_get()->notify_window_period);
...
{
	/* play sound */
	if (prefs_common_get()->enable_newmsg_notify_sound &&
	    prefs_common_get()->newmsg_notify_sound) {
		play_sound(prefs_common_get()->newmsg_notify_sound, TRUE);
	}

	/* execute command */
	if (prefs_common_get()->enable_newmsg_notify &&
	    prefs_common_get()->newmsg_notify_cmd) {
		gchar buf[1024];

		if (str_find_format_times
			(prefs_common_get()->newmsg_notify_cmd, 'd') == 1)
			g_snprintf(buf, sizeof(buf),
				   prefs_common_get()->newmsg_notify_cmd,
				   summaries.total_msgs);
		else
			strncpy2(buf, prefs_common_get()->newmsg_notify_cmd,
				 sizeof(buf));
...
static gint imap_recv_msg(Session *_session, const gchar *msg)
{
	...
	if (!msg || msg[0] == ')') {
		...
		if (summary && prefs_common_get()->enable_newmsg_notify_window) {
